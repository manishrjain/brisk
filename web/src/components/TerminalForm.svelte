<script lang="ts">
  ...
  $: {
    ...
    fields = [
      ...
      { key: 'scenario', label: 'Scenario', help: 'buy_vs_rent: compare buying vs renting | sell_vs_keep: compare selling vs keeping | payoff_vs_invest: compare paying extra on mortgage vs investing', placeholder: 'buy_vs_rent', visible: () => true, toggleValues: ['buy_vs_rent', 'sell_vs_keep', 'payoff_vs_invest'], fieldType: 'toggle' },

      { key: 'header_economic', label: '', help: '', placeholder: '', visible: () => true, isHeader: true, headerText: 'ECONOMIC ASSUMPTIONS' },
      { key: 'inflationRate', label: 'Inflation Rate (%)', help: 'Annual inflation for all recurring costs', placeholder: '3', visible: () => formInputs.scenario !== 'payoff_vs_invest', fieldType: 'rate' },
      ...
      { key: 'header_asset', label: '', help: '', placeholder: '', visible: () => formInputs.scenario !== 'payoff_vs_invest', isHeader: true, headerText: formInputs.scenario === 'sell_vs_keep' ? 'ASSET' : 'BUYING' },
      { key: 'purchasePrice', label: formInputs.scenario === 'sell_vs_keep' ? 'Original Purchase Price' : 'Asset Purchase Price', help: formInputs.scenario === 'sell_vs_keep' ? 'What you originally paid for the asset (for capital gains)' : 'Initial purchase price of the asset', placeholder: '500k', visible: () => formInputs.scenario !== 'payoff_vs_invest', fieldType: 'currency' },
      ...
      { key: 'annualInsurance', label: 'Annual Tax & Insurance', help: formInputs.scenario === 'sell_vs_keep' ? 'Yearly costs if keeping' : 'Yearly insurance cost', placeholder: '3k', visible: () => formInputs.scenario !== 'payoff_vs_invest', fieldType: 'currency' },
      { key: 'annualTaxes', label: 'Other Annual Costs', help: 'Maintenance, utilities, HOA, etc.', placeholder: '5k', visible: () => formInputs.scenario !== 'payoff_vs_invest', fieldType: 'currency' },
      { key: 'annualIncome', label: 'Annual Income from Asset', help: 'Rental income or other income generated by the asset', placeholder: '0', visible: () => formInputs.scenario !== 'payoff_vs_invest', fieldType: 'currency' },
      { key: 'appreciationRate', label: 'Appreciation Rate (%)', help: formInputs.scenario === 'sell_vs_keep' ? 'Annual rate if keeping. Comma-separated for different years' : 'Annual rate (can be negative for depreciation). Comma-separated values apply to first years, last value for all remaining years (e.g., \'10,5,3\' = 10% yr1, 5% yr2, 3% yr3+)', placeholder: '3', visible: () => formInputs.scenario !== 'payoff_vs_invest', fieldType: 'rate' },
      ...
      { key: 'loanAmount', label: formInputs.scenario === 'sell_vs_keep' ? (formInputs.includeRefinance === 'yes' ? 'Refinance Loan Amount' : 'Original Loan Amount') : (formInputs.scenario === 'payoff_vs_invest' ? 'Original Loan Amount' : 'Loan Amount'), help: formInputs.scenario === 'sell_vs_keep' || formInputs.scenario === 'payoff_vs_invest' ? (formInputs.includeRefinance === 'yes' ? 'New refinance loan amount' : 'The original loan amount when purchased (we\'ll calculate remaining balance)') : 'Total mortgage/loan amount', placeholder: '400k', visible: () => true, fieldType: 'currency' },
      ...
      { key: 'remainingLoanTerm', label: 'Remaining Loan Term', help: 'Time left on loan (e.g., 25y)', placeholder: '25y', visible: () => (formInputs.scenario === 'sell_vs_keep' && formInputs.includeRefinance !== 'yes') || formInputs.scenario === 'payoff_vs_invest', fieldType: 'duration' },
      ...
      { key: 'rentDeposit', label: 'Rental Deposit', help: formInputs.scenario === 'sell_vs_keep' ? 'Initial rental deposit if selling' : 'Initial rental deposit', placeholder: '5k', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.scenario === 'sell_vs_keep' && formInputs.includeRentingSell !== 'yes', fieldType: 'currency' },
      { key: 'monthlyRent', label: 'Monthly Rent', help: formInputs.scenario === 'sell_vs_keep' ? 'Monthly rent if selling' : 'Base monthly rent amount', placeholder: '3k', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.scenario === 'sell_vs_keep' && formInputs.includeRentingSell !== 'yes', fieldType: 'currency' },
      { key: 'annualRentCosts', label: 'Annual Rent Costs', help: formInputs.scenario === 'sell_vs_keep' ? 'Yearly rental costs if selling' : 'Yearly rental-related costs', placeholder: '1k', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.scenario === 'sell_vs_keep' && formInputs.includeRentingSell !== 'yes', fieldType: 'currency' },
      ...
      { key: 'header_selling', label: '', help: '', placeholder: '', visible: () => formInputs.scenario !== 'payoff_vs_invest', isHeader: true, headerText: 'SELLING' },
      { key: 'includeSelling', label: 'Include Selling Analysis', help: 'Toggle to enable/disable selling analysis', placeholder: 'no', visible: () => formInputs.scenario !== 'payoff_vs_invest', toggleValues: ['yes', 'no'], disabled: () => formInputs.scenario === 'sell_vs_keep', fieldType: 'toggle' },
      { key: 'agentCommission', label: 'Agent Commission (%)', help: 'Percentage of sale price paid to agents', placeholder: '6', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.includeSelling !== 'yes', fieldType: 'rate' },
      { key: 'stagingCosts', label: 'Staging/Selling Costs', help: 'Fixed costs to prepare and sell', placeholder: '10k', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.includeSelling !== 'yes', fieldType: 'currency' },
      { key: 'taxFreeLimits', label: 'Tax-Free Gains Limit', help: 'Capital gains exempt from tax. Comma-separated for different years (e.g., \'500k,0k\' = 500k year 1, 0 year 2+)', placeholder: '250k', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.includeSelling !== 'yes', fieldType: 'currency' },
      { key: 'capitalGainsTax', label: 'Capital Gains Tax (%)', help: 'Long-term capital gains tax rate', placeholder: '20', visible: () => formInputs.scenario !== 'payoff_vs_invest', disabled: () => formInputs.includeSelling !== 'yes', fieldType: 'rate' },
    ];
  }
  ...
  function toggleFieldValue(field: FormField) {
    ...
    const currentIndex = field.toggleValues.indexOf(currentValue);

    // Cycle to next value
    const nextIndex = (currentIndex + 1) % field.toggleValues.length;
    formInputs[field.key] = field.toggleValues[nextIndex];
  }
